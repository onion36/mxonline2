...
class ModifyPwdView(View):
    def post(self, request):
        ...
        if modify_form.is_valid():
            ...
            email = request.POST.get('email', '') #因为在if判断中，需要重新获取
            return render(request, 'password_reset.html', {'email': email, 'err_msg':"密码不一致，重新输入"})
...
class IndexView(View):
    ...
    def get(self, request):
        ...
        courses = Course.objects.filter(is_banner=False)[:6] #非轮播图
